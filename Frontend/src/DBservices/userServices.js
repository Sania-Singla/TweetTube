export class UserServices {
    ...
    async registerUser(inputs,setData,setLoading) {   //⭐ we dont need any setData state 
        try {
            setLoading(true) 
            // const newInputs = {
            //     ...inputs,  //wrong
    ...
    async getCurrentUser() {
        try 
        {
            ...
            console.log(data);
            if (res.ok) {
                return data;   //for this we didnt do .then(because we can't return in .then syntax)
            }
            else if(res.status !== 500)   //⭐⭐ALL AUTH ERRORS ARE HANDLED HERE NO NEED TO HANDLE ANYWHERE ELSE
            {
                return data;
            }
            else if(res.status === 500) {
