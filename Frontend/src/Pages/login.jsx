...
export default function LoginPage() {
    ...
    const inputElements = inputFields.map(input => (
        <div className="w-[320px] mb-5" key={input.id}>
            <div className="mb-[1px]">{input.required && <span className="text-red-600 mr-[2px]">*</span>}<label htmlFor={input.id}>{input.label}</label></div>
            <input 
                type={input.type} 
    ...
    async function handleSubmit(e) {
        ...
        else    //because req gyi hai becuase agr details hi entered nhi thi toh hmne request bhene hi nhi dii  //so user is not found message received from backend
        {  
        ...
        <div className="fixed inset-0 bg-[#0c0c0c] z-[150] flex flex-col items-center justify-center">
            <div className="flex flex-col items-center justify-center">
                <div className="flex flex-col items-center justify-center w-full">
                    ...
                    <h3 className="text-lg mt-1">Don't have an Account yet ?<Link to="/register" className="text-[1rem] text-blue-700"> Sign up</Link></h3>
                    <motion.div initial={{width:0}} animate={{width:"120%"}} transition={{duration:0.3}} className=" h-[0.005rem] mt-2 bg-[#9e9e9e]"></motion.div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="mt-3 flex flex-col items-start justify-center">
                    ...
                    <div className="w-full text-center mt-4">
                        <button type="submit" disabled={loading} className="disabled:cursor-not-allowed w-full bg-[#8871ee] text-lg h-12 border-[0.09rem] border-[#b5b4b4] border-solid hover:border-dotted hover:bg-[#6856b5]">
                        {
                            loading ? 
