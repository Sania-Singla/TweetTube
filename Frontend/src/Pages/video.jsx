...
function VideoPage() {
  ...
  useEffect(()=>{
    ...
    setLoading(true);
    videoServices.getVideoById(videoId)
    .then(res=>{
  ...
  function toggleSubscribe() {
    channelServices.toggleSubscribe(data.owner._id)
    .then(res=>{ 
  ...
  return (
    <div onClick={(e)=>{
      ...
      {!loading &&
        <div className="flex flex-col lg:flex-row items-start justify-center gap-6 w-full">  {/*full screen*/}
          <div className="flex flex-col items-center justify-center w-full lg:w-[80%]">
            ...
            <div onClick={()=>setExpandDescription(true)} ref={bigParentRef} className="border-[0.01rem] border-[#b5b4b4] bg-[#0c0c0c] rounded-lg cursor-pointer p-4 w-full mt-2 hover:bg-[#191919]">  
              <div className="flex">
                {/* title */}
                <div className="flex flex-col items-start justify-center w-full">
                  <div className="w-full line-clamp-2 text-[1.3rem] leading-6 font-medium">{data.title}</div>
                  <div className="w-full mt-1 text-gray-400 text-[0.9rem]">{data.views} views &bull; {formatDistanceToNow(parseISO(data.createdAt), { addSuffix: true })}</div>
                </div>
                  ...
                  <div className="ml-6">
                    ...
                    (userData?.username !== data.owner.username && 
                      <div className="flex items-center justify-center rounded-lg bg-[#212121] w-fit h-fit overflow-hidden border-[0.01rem] border-[#b5b4b4]">
                        <div  onClick={handleLikeClick} className="cursor-pointer flex items-center justify-center border-r-[0.01rem] border-[#b5b4b4] hover:bg-[#3f3f3f] px-[5px] py-[6px]">
                          <button className="ml-2">
                            <FontAwesomeIcon size="lg" icon={likedStatus ? faSolidThumbsUp : faRegularThumbsUp} color={'white'} />
                          </button>
                          <p className="mx-[8px]">{likesCount}</p>
                        </div>

                        <div onClick={handleDislikeClick} className="cursor-pointer flex items-center justify-center px-[5px] py-[6px] hover:bg-[#3f3f3f]">
                          <button className="transform -scale-x-100 ml-2">  
                            <FontAwesomeIcon size="lg" icon={disLikedStatus ? faSolidThumbsDown : faRegularThumbsDown} color={'white'} />
                          </button>
                  ...
                  <div className="relative min-w-[90px]">
                    {userData ? 
                      ...
                      <div className="border-[0.01rem] border-[#b5b4b4] rounded-lg overflow-hidden">
                        <div onClick={()=>setPlaylistPopup(true)} className="cursor-pointer w-fit p-[5px] px-3 bg-[#212121] hover:bg-[#3f3f3f]">
                          <button className="size-[27px] fill-[#f1f1f1] w-full rounded-lg flex items-center justify-center">{icons.save} <span className="text-lg pb-[3px] ml-1">save</span></button>
                        </div>
                      </div>):
                      (
                        <div className="border-[0.01rem] border-[#b5b4b4] rounded-lg overflow-hidden">
                      ...
                      (
                        userData.username !== data.owner.username ?
                          ...
                          (
                            ...
                            >
                              <i className="fa-solid fa-user-pen text-xl"></i><span className="font-semibold  ml-[6px]">Edit</span>
                            </button> 
                          )
                      ) : 
                      ( <button 
                ...
                <div className="md:hidden flex items-center justify-between w-full">
                  ...
                  <div className="relative">
                    {userData ? 
                      (
                      <div className="border-[0.01rem] border-[#b5b4b4] rounded-lg overflow-hidden">
                        <div onClick={()=>setPlaylistPopup(true)} className="cursor-pointer w-fit py-[5px] px-3 bg-[#212121] hover:bg-[#3f3f3f]">
                          <button className="size-[27px] fill-[#f1f1f1] w-full rounded-lg flex items-center justify-center">{icons.save} <span className="text-lg pb-[3px] ml-1">save</span></button>
                        </div>
                      </div>):
                      (
            ...
            {/* comment section */}
            <div onClick={()=>setExpandComments(true)} ref={commentRef}  className="cursor-pointer p-4 text-white w-full bg-[#0c0c0c] mt-4 border-[0.01rem] border-[#b5b4b4] rounded-lg">
          ...
          {
            loginPopup && 
            <div ref={loginRef} onClick={closeLoginPopup} className="fixed inset-0 backdrop-blur-sm z-[150] flex flex-col items-center justify-center">
              <LoginPopup close={()=>setLoginPopup(false)} popupText={loginPopupText}/>
            </div>
          }

          {/* side recemondations */}
          <div className="lg:w-[35%] h-full w-full"><SideVideos/></div>   
        </div>
      }
    </div>
  )
