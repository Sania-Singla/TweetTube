...
import {VideoList,PulseVideoList} from "../index";
...
function WatchHistory() {
    ...
    useEffect(()=>{
        if(loginStatus)
        {
            if(page===1 || history.length !== historyInfo.totalVideos)   //not condn because when we enter gibberish the any term then previous historyInfo.totalVideos and length both were 0 so equal and onclick length =0 vo toh pehle bhi tha so still equal thats why explicitly consider that 0 case
            {
                setLoading(true);
                channelServices.getWatchHistory(setLoading,setHistoryInfo,setHistory,page,search,10)
                .then(data => {
        ...
        if(!videosFound) return <div>no watched videos</div> 
            ...
            <div>
                <div className="w-full h-full">
                    <div className="bg-[#0c0c0c] mb-8 flex items-center justify-between">   {/*sticky top-[calc(80px-0.01rem)] z-[120]*/}
                        <div className='px-2 w-[60%] h-10 flex items-center'>
                            <div className='bg-transparent text-[#b5b4b4] h-full flex items-center pt-4 justify-center text-xl mr-3'><i className="fa-solid fa-magnifying-glass"></i></div>
                            <motion.input 
                                type="text" 
                                placeholder='Search'
                                value={search}
                        ...
                        <button onClick={clearWatchHistory} className="bg-[#8871ee] group hover:bg-slate-900 border-transparent border-dotted hover:border-[#b5b4b4] border-[0.01rem] rounded-md relative top-[7px] right-[7px]">
