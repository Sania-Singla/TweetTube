...
export default function UploadVideoPopup({setController,close,setUploadVideoPopup,setFileSize,setUploadingPopup,setVideoUploadedPopup}) {
    ...
    const handleMouseOver = () => {
        if(!inputs.title || !inputs.description || !inputs.videoFile || !inputs.thumbnail || error.root || error.title || error.description || error.thumbnail || error.videoFile) return setDisabled(true);
    ...
    async function handleUploadVideo(e) {
        ...
        // Create and set a new AbortController
        ...
        // }
        // setController(null); // Cancel the request if the component unmounts
        ...
        <div className="bg-[#0e0e0e] border-[0.1rem] border-[#b5b4b4] rounded-md shadow-black shadow-lg w-[600px]">
            ...
            <form onSubmit={handleUploadVideo} className="w-full p-4">
                ...
                <div className="p-6 border-dashed border-[0.1rem] border-[#d4d4d4] w-full flex flex-col items-center justify-center">
                    ...
                    <div className="mt-4">
                        <button type="button" onClick={(e)=>videoFileRef.current.click()} className="bg-[#8871ee] border-[0.01rem] border-transparent hover:border-[#b5b4b4] p-[5px] w-[100px] hover:font-medium text-black text-lg">Select File</button>
                    </div>
                </div>
                {error.videoFile && <div>
