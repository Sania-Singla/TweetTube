...
function Header() {
  ...
  const navElements = navItems.map(item => (
    item.show && 
    <li key={item.name} value={item.name} className="bg-[#8871ee] text-[#0c0c0c] text-center h-full w-[70px] flex items-center justify-center text-lg font-semibold  border-[0.01rem] border-[#b5b4b4] active:border-white ">
      <button 
