...
import { useChannelHook } from '../../hooks';
  ...
  useEffect(()=>{
    ...
    {
      setLoading(true);
      channelServices.getUserVideos(setVideoInfo,videos,setVideos,setLoading,channelData._id,page,10)
      .then(videos=>{
    ...
    <div className='pt-[8px]'>
      { //because 1 or 2 videos were as grid auto-fit do is that evenly spearding (if only video was covering whole screen so we needed to restrict the col width)
        ...
        (
          ...
          <div className='grid grid-cols-[repeat(auto-fit,_minmax(320px,_1fr))] gap-4 xl:grid-cols-[repeat(3,minmax(320px,1fr))]'>{videoElements}</div>
        ) :  //means allVideos.length === 1
        (
          <div className='grid grid-cols-[repeat(auto-fit,_minmax(320px,_1fr))] gap-4 md:grid-cols-[repeat(3,minmax(350px,1fr))]'>{videoElements}</div>
        )
      }

      {
        (loading && page === 1) &&
        <div className='grid grid-cols-[repeat(auto-fit,_minmax(320px,_1fr))] gap-x-4'>{pulseEffect}</div>
      }
