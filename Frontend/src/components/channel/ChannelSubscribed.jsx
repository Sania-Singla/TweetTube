...
function ChannelSubscribed() {
    ...
    useEffect(()=>{
        channelServices.getSubscribedChannels(setLoading,channelData._id)
        .then(res=>{
            if(res && res.length>0)
            {
                setSubscribedChannels(res.map(channel=>({
                    ...channel,
    ...
    const handleSubscribeClick = (_id)=>{
        channelServices.toggleSubscribe(_id)
        .then(res=>{ 
            if(res)
            {
    ...
    const subscribersElements = subscribedChannels?.filter(channel=>{
        if(search === "") return channel;
        else if(channel.fullname.includes(search)) return channel;
    }).map( channel =>{
        const {_id,avatar,fullname,username,subscribersCount,subscribedStatus} = channel;

        return (
        <div className='flex items-center mb-4 relative' key={_id}>
            <div className='flex items-center justify-center cursor-pointer' onClick={()=>navigate(`/channel/${username}`)}>
                <div className='w-[50px] rounded-full overflow-hidden h-[50px] md:h-[60px] md:w-[60px]'>
                    <img src={avatar} className='w-full h-full object-cover'/>
                </div>

                <div className='ml-3'>
                    <div className='text-[1.3rem] md:text-[1.4rem] '>{fullname}</div>
                    <div className='text-[0.9rem] text-[#b5b4b4]'>{subscribersCount} Subscribers</div>
                </div>
            </div>
    ...
    if(!subscribedFound) return <div>no subscriberd channels !</div>   //loading is already false then 
    else return (
