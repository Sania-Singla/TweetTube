...
import {PulseChannel, LoginPopup } from ".."
...
function Channel() {
  ...
  useEffect(()=>{
    channelServices.getChannelProfile(setLoading,username)
    .then(res => {
  ...
  const optionsElements = options.map(option => (
    <div key={option.name} className="flex items-center justify-center w-1/5 mx-[3px]">
      <NavLink to={option.path} end className={({isActive}) => `${isActive?"bg-white font-semibold border-[#8871ee] rounded-t-md text-[#0c0c0c]" : "bg-[#0c0c0c] border-white text-[#8871ee]" } w-full h-full border-b-[0.13rem] text-center text-md md:text-lg py-1`} >
        {option.name}
      </NavLink>
    </div>
  ))
  ...
  function toggleSubscribe() {
    channelServices.toggleSubscribe(data._id)
    .then(res => setIsSubscribed(prev=>{
      if(prev) {
    ...
    <div className='text-white'>
      ...
      <div className="relative top-[-15px] flex items-center ml-1">
        ...
        <div className="ml-[15px]">
          ...
          {
            loginPopup && 
            <div ref={ref} onClick={closeLoginPopup} className="fixed inset-0 backdrop-blur-sm z-[150] flex items-center justify-center">
