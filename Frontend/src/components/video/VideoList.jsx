...
function VideoList({
    ...
    isSearchResultVideoList = false,
    removeFromLikedVideos = null,
}) {
    const {
        _id,
        title,
    ...
    return (
        ...
        <div
            ...
            className="flex flex-col sm:flex-row relative mb-4 hover:bg-[#2121219d] pb-2 sm:pb-0 cursor-pointer"
        >
            <div>
            ...
            <div className="hidden sm:block mt-1 sm:mt-0 sm:ml-3">
                ...
                <div
                    className="text-[15.5px] text-[#b0afaf] line-clamp-2 mt-[5px]" /* or => overflow-hidden whitespace-nowrap text-ellipsis (remeber the parent container should be rescticted to width (max-w-))*/
                >
                    {description}
                </div>
                ...
                {!isSearchResultVideoList && !isWatchHistory && (
                    <div className="w-fit flex justify-center items-center absolute right-2 top-2 text-[#c3c2c2]">
                        <button
                            className="w-7 h-7 text-2xl flex items-center justify-center"
                            onClick={(e) => {
                                e.stopPropagation();
                                removeFromLikedVideos(_id);
                            }}
                        >
                            <div className="fill-none hover:stroke-[#d90303] stroke-[#a40000] size-full">
                                {icons.delete}
                            </div>
                        </button>
                    </div>
            ...
            <div className="ml-2 mt-2 sm:hidden flex items-start">
                ...
                <div className="ml-3 leading-[22px] w-full relative ">
                    <div
                        className={`text-[1.3rem] font-medium text-white line-clamp-2 ${isWatchHistory ? '' : 'pr-10'} leading-6`}
                    >
                        {title}
                    </div>
