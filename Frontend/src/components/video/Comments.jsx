...
export default function Comments({expandComments}) {
    ...
    useEffect(()=>{
        ...
        {
            setLoading(true);
            commentServices.getVideoComments(setCommentsInfo,comments,setComments,page,10,videoId)
            .then(res=>{
                if(res && res.length > 0) {
    ...
    const commentElements = comments?.map((comment,index)=>{
        ...
        if(comments.length === index+1) return (
                <div key={_id} ref={callbackRef} className="">
                    <hr className="mt-4 mb-3"/>
        
                    <div className="flex items-start justify-start">
                        <div className="pt-[10px] cursor-pointer" onClick={()=>navigate(`/channel/${username}`)}>
                            <div className="size-10 rounded-full overflow-hidden"><img src={avatar} alt={fullname} className="w-full h-full object-cover" /></div>
                        </div>
        
                        <div className="ml-3 w-full relative" onClick={()=>setExpandCommentId(prev=>prev?"":_id)}>
                            <div className="text-md text-[#e9e9e9] text-[1.1rem]">{fullname} <span className="text-[0.5rem] h-full align-middle">&bull;</span> <span className="text-[0.8rem] text-[#c6c6c6]">{formattedCreatedAt}</span></div>
                            ...
                            <div className="mt-3">
                                ...
                                (<div className="flex items-center justify-center rounded-lg bg-[#212121] w-fit h-fit overflow-hidden border-[0.01rem] border-[#b5b4b4]">
                                    <div  onClick={()=>handleLikeClick(_id)} className="cursor-pointer flex items-center justify-center border-r-[0.01rem] border-[#b5b4b4] hover:bg-[#3f3f3f] py-[3px]">
                                    ...
                                    <div onClick={()=>handleDislikeClick(_id)} className="cursor-pointer flex items-center justify-center py-[3px] hover:bg-[#3f3f3f]">
                                        <button className="transform -scale-x-100 ml-2">  
                                        <FontAwesomeIcon size="lg" icon={hasDisliked ? faSolidThumbsDown : faRegularThumbsDown} color={'white'} />
                                        </button>
...

                            {
                                ...
                                <div className="absolute top-0 right-0 pl-4">
                                    {currentEditId!==_id && <button onClick={()=>{
                                        setCurrentEditId(_id);
                                        setEditComment(content);
                                    }} className="size-[31px] fill-none stroke-[#b5b4b4] mr-2 border-[0.01rem] border-transparent rounded-lg p-[5px] hover:border-[#b5b4b4]">{icons.edit}</button>}
                                    <button onClick={()=>deleteComment(_id)} className="size-[33px] fill-none stroke-[#b5b4b4] border-[0.01rem] border-transparent rounded-lg p-[5px] hover:border-[#b5b4b4]">{icons.dustbin}</button>
                                </div>
                            }
                        </div>
                    </div>
    ...
    async function toggleLike(commentId,toggleStatus) {
        ...
        if(res)
        {
            if(toggleStatus)
            {
                setComments(prevComments=>(  //array
                    prevComments.map(comment=>{
                        if(comment._id === commentId)
                        {
                            ...
                            else if(comment.hasDisliked) return {
                                ...comment,
                                hasLiked: true,
                                hasDisliked:false, 
                                likes: comment.likes+1,
            ...
            else
            {
                setComments(prevComments=>(  //array
                    prevComments.map(comment=>{
                        if(comment._id=== commentId)
                        {
                            if(comment.hasDisliked) return {
                                ...comment,
                                hasLiked: false,  //no need though
                                hasDisliked:false, 
                                dislikes: comment.dislikes-1
                            } 
                            else if(comment.hasLiked) return {
                                ...comment,
                                hasLiked:false,
                                hasDisliked:true,
                                likes: comment.likes-1,
        ...
        <div className="mt-4 mb-5 flex items-center justify-center border-[0.01rem] border-[#e7e7e7] rounded-lg px-3">
            ...
            <div className="flex items-center gap-2">
                ...
                <button 
                    ...
                    type="submit" className="border-[0.01rem] border-[#b5b4b4] rounded-full pb-[2px] bg-[#8871ee] hover:bg-[#7460ca] text-black font-semibold px-[10px]"
                    >
                    comment
                </button>
            </div>
        ...
        <div>
            <div>
                <div>
                    ...
                    <div className={`${expandComments ? "flex" : "hidden"} lg:flex mt-4 mb-5 items-center justify-center border-[0.01rem] border-[#e7e7e7] rounded-lg px-3`}>
                        <input 
                            type="text"
                            name="newComment" 
                            id="newComment"
