...
export default function RecemendationVideoList({video,reference}) {
    ...
    return (//important⭐adding a "/" can make a route absolute prevent appending
        <div ref={reference} onClick={()=>navigate(`/video/${_id}`)} className="flex items-start justify-start relative mb-3 hover:bg-[#2121219d] cursor-pointer">
            <div className="relative w-[215px] pt-1">
                <div className="w-[215px] h-[135px]  rounded-xl overflow-hidden"><img src={thumbnail} alt={title} className="w-full h-full object-cover"/></div>
                <div className="bg-[#0c0c0c] opacity-90 text-white w-fit px-[6px] pb-[1px] rounded-md text-[0.95rem] absolute bottom-2 right-2">{formattedDuration}</div>
            </div>
            ...
            <div className="ml-3">  
                ...
                <div className="text-[0.9rem] text-[#b7b7b7] mt-[2px]">{views} views &bull; {formattedCreatedAt}</div>
            </div>
        </div>
    )
}