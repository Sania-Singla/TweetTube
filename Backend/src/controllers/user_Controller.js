...
const change_Current_Password = async (req, res) => {
    try {
        ...
        if (!oldPassword || !newPassword || !confirmedPassword) return res.status(400).json({ message: "MISSING_FIELDS" });
        //check for new===old
        if (oldPassword === newPassword) return res.status(400).json({ message: "OLD_MATCH_NEW" });
        //check for new===confirm
        if (newPassword !== confirmedPassword) return res.status(400).json({ message: "CONFIRM_DOESNT_MATCH_NEW" });
...
const update_Account_Details = async (req, res) => {
    try {
        ...
        // check for empty fields
        if (!fullname || !email || !password) return res.status(400).json({ message: "MISSING_FIELDS" });
...
const update_Channel_Info = async (req, res) => {
    try {
        ...
        // check for empty fields
        if (!username || !description || !password) return res.status(400).json({ message: "MISSING_FIELDS" });
...
const clear_Watch_History = async (req, res) => {
    try {
        ...
        return res.status(500).json({
            message: "something bad happened while clearing out the watch history",
            err: err.message,
        });
    }
};
