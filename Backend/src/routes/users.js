...
userRouter.route("/logout").post(verifyJWT, logout_User);

userRouter.route("/refresh-token").post(refresh_Access_Token);

userRouter.route("/change-password").patch(verifyJWT, change_Current_Password);

userRouter.route("/current-user").get(verifyJWT, get_Current_User);
...
userRouter.route("/update-channel-info").patch(verifyJWT, update_Channel_Info);
...
userRouter
    ...
    .get(optionalVerifyJWT, get_Channel_Profile);

userRouter.route("/watch-History").get(verifyJWT, get_watch_history);

userRouter.route("/clear-History").delete(verifyJWT, clear_Watch_History);
